...
const PaymentReceiptScreen = ({ navigation }) => {
    ...
    // Fetch Customers
    useEffect(() => {
        if(jwtToken && selectedAssetLocation){
            fetchCustomers();
        }   
    }, [jwtToken, selectedAssetLocation]);   

    const fetchCustomers = async () => {
        try {
            await axios.get(`${BASE_URL}/api/customers_for_paymentreceipt?projectName=${selectedProjectName}&assetLocation=${selectedAssetLocation}`, { headers: headers })
            .then(response => {
    ...
    useEffect(() => {
        if(jwtToken && selectedProjectName){
          fetchAssetLocations();
        }   
    }, [jwtToken, selectedProjectName]);   

    const fetchAssetLocations = async () => {
        try {
            await axios.get(`${BASE_URL}/api/asset_locations?projectName=${selectedProjectName}`, { headers: headers })
            .then(response => {
                console.log("AssestLocations :", response.data.locations);
                setAssetLocations(response.data.locations);
            }).catch(error => {
                if (error.response) {
                    // Handle response-related errors
                    console.error('Response:', error.response);
            ...
            if (jwtToken) {
                const response = await axios.get(`${BASE_URL}/api/paymentreceipts`, {
                    params: {
                        ...
                        asset_location:selectedAssetLocation
                    },
                    headers: headers
                });
                console.log("paymentreceipts :", response.data.paymentreceipts);
        ...
        <html>
            ...
            <div class="receipt">
                ...
                <h1>Payment Receipt</h1>
                    ...
                    <p><span><strong>Amount Paid:</strong> ${payment.plot_number}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span><strong>Phase:</strong> ${payment.payment_type}</span></p>
                    <p><span><strong>Amount Paid:</strong> ${payment.down_payment_amount}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span><strong>Phase:</strong> ${payment.phase}</span></p>
                <div class="footer">
                    <!DOCTYPE html>
                    <html lang="en">
                    <head>
            ...
            <View key={index}>
                ...
                >
                    ...
                    <Text style={styles.receiptText}>
                        ...
                        {' '} Location:<Text style={{ color: 'brown' }}>{payment.asset_location}</Text>
                    </Text>
                    <Text style={styles.receiptText}>
                        Plot Number: <Text style={{ color: 'blue' }}>{payment.plot_number}</Text>
                        {' '} Payment Type: <Text style={{ color: '#11998e' }}>{payment.payment_type}</Text>
                    </Text>
                    <Text style={styles.receiptText}>
                ...
                <View style={styles.pageContent}>
                    ...
                    <View style={styles.InputRow}>
                        <ProjectPicker
                            ...
                            onValueChange={(itemValue) => { setSelectedProjectName(itemValue),                           
                                fetchAssetLocations()                             
                            }}
                        />
                    </View>                   
                    {selectedProjectName && (
                        <View style={styles.InputRow}>
                            <LocationPicker
                                selectedValue={selectedAssetLocation}
                                onValueChange={(itemValue) => {
                                    setSelectedAssetLocation(itemValue),
                                    fetchCustomers()
                                }}
                                selectedProjectName ={selectedProjectName}                               
                                fetchedAssetLocations={assetLocations}
                            />
                        </View>
                    )}
                    {selectedProjectName && selectedAssetLocation && (
                        <View style={styles.InputRow}>
                            <CustomerPicker
